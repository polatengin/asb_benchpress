...
function New-Deployment {
  ...
  az deployment group create --resource-group $ResourceGroup --template-file $TemplateFile --parameters $Parameters --output "none"
}
...
function Deploy-DemoApp {
  ...
  if ($Confirmed -eq "y") {
    ...
    New-Deployment -ResourceGroup "benchpress-rg-${EnvironmentSuffix}" -TemplateFile "./demo/main.bicep" -Parameters "suffix=${EnvironmentSuffix}"
    ...
    az webapp up --name "benchpress-web-${EnvironmentSuffix}" --resource-group "benchpress-rg-${EnvironmentSuffix}" --location "${Location}" --sku "F1" --output "none"
    
    Pop-Location
  }

  Remove-Item -Path .\AspNetCore.Docs -Recurse -Force
}
...
Write-Information "To run the tests, you need to set the following environment variables:"
Write-Information "$$env:ENVIRONMENT_SUFFIX=${ENVIRONMENT_SUFFIX}"
Write-Information "$$env:LOCATION=${LOCATION}"
